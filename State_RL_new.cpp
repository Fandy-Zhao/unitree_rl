...
void State_RL::_observations_compute()
{
    ...
    this->_observation = torch::cat({
        ...
        dof_vel_tensor.view({1, -1}) * scale_dof_vel, //关节速度，需要重排序并应用缩放因子
        _action.view({1, -1})
        }, -1);

    //加入隐式信息 - 添加额外状态信息
    torch::Tensor priv_explicit = torch::cat({
        //实际线速度，应用缩放因子
        //占位零向量（与线速度形状相同）
        //占位零向量（与线速度形状相同）
    },-1)
    ...
    //潜在的系统参数扰动信息
    torch::Tensor priv_latent = torch::cat({
        // this->mass_params_tensor,        #质量扰动参数
        // this->friction_coeffs_tensor,    #摩擦系数扰动参数
        // this->motor_strength[0] - 1,     #电机最大输出力矩扰动参数（归一化到0附近）
        // this->motor_strength[1] - 1      #第二个电机强度参数（同样归一化到0附近）
    },-1)
