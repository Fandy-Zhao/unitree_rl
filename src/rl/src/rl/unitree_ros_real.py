...
import rospy
from unitree_legged_msgs.msg import (
    LowState,
    LowCmd
)
...
# 注意：在ROS Noetic中，我们直接导入或实现CRC计算
try:
    from crc_module import get_crc
except ImportError:
    # 如果没有crc_module，使用一个简单的实现
    def get_crc(msg):
        return 0  # 简化的CRC计算
...
class UnitreeRosReal:
    ...
    def __init__(self,
            ...
            low_state_topic= "/rl/lowstate",
            low_cmd_topic= "/rl/lowcmd",
        ...
        rospy.init_node("unitree_ros_real", anonymous=True)
    ...
    def parse_config(self):
        ...
        rospy.loginfo("[Env] action scale: {:.2f}".format(self.action_scale))
    ...
    def start_ros_handlers(self):
        ...
        """
        # ROS publishers
        self.low_cmd_pub = rospy.Publisher(
            self.low_cmd_topic,
            LowCmd,
            queue_size=1
        )
        ...
        #     String,  # 使用String消息类型代替Request
        #     queue_size=1,
        # )

        # self.sport_mode_pub = rospy.Publisher(
        #     '/api/sport/request',
        #     String,  # 使用String消息类型代替Request
        #     queue_size=1,
        # )
        ...
        rospy.loginfo("ROS handlers started, waiting to recieve critical low state and wireless controller messages.")
        if not self.dryrun:
            rospy.logwarn(f"You are running the code in no-dryrun mode and publishing to '{self.low_cmd_topic}', Please keep safe.")
        else:
            rospy.logwarn(f"You are publishing low cmd to '{self.low_cmd_topic}' because of dryrun mode, Please check and be safe.")
        
        # 等待接收消息
        rospy.wait_for_message(self.low_state_topic, LowState)
        # rospy.wait_for_message(self.joy_stick_topic, WirelessController)
        rospy.loginfo("Low state and wireless message received, the robot is ready to go.")

    """ ROS callbacks and handlers that update the buffer """

    def _low_state_callback(self, msg):
        # rospy.logwarn("Low state message received.")
        ...
        for sim_idx in range(self.NUM_DOF):
            ...
            self.dof_pos_[0, sim_idx] = self.low_state_buffer.motorState[real_idx].q * self.dof_signs[sim_idx]
        for sim_idx in range(self.NUM_DOF):
            real_idx = self.dof_map[sim_idx]
    ...
    def _sport_state_change(self, mode):
        ...
        if mode == 0:
            parameter = '{"name":"sport_mode","switch":0}'
        elif mode == 1:
            parameter = '{"name":"sport_mode","switch":1}'
        
        api_data = {
            "header": {
                "identity": {"id": 0, "api_id": 1001},
                "lease": {"id": 0},
                "policy": {"priority": 0, "noreply": False}
            },
            "parameter": parameter,
    ...
    def _get_ang_vel_obs(self):
        ang_vel = torch.from_numpy(np.array(self.low_state_buffer.imu.gyroscope)).unsqueeze(0).to(device=self.model_device, dtype=torch.float32)
    ...
    def get_stand_action(self):
        if self.firstRun:
            for i in range(12):
                self.startPos[i] = self.low_state_buffer.motorState[i].q
            self.firstRun = False
        ...
        if self.percent_1 < 1:
            ...
            if self.firstrun_target_1:
                rospy.loginfo('Going to target Pos 1.')
        ...
        if (self.percent_1 == 1) and (self.percent_2 <= 1):
            ...
            rospy.loginfo('Staying in target Pos 2.')
